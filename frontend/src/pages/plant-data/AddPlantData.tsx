...
import { plantDataApi, PlantRecord, FormulaVariable, BulkUploadResponse } from '@/services/plantDataApi';
...
const AddPlantData = () => {
  ...
  const [activeTab, setActiveTab] = useState<string>(location.state?.initialTab === 'excel' ? 'excel' : 'manual');
  const [isProcessingBulkUpload, setIsProcessingBulkUpload] = useState(false);
  const [bulkUploadSuccess, setBulkUploadSuccess] = useState<boolean | undefined>(undefined);
  const [bulkUploadStats, setBulkUploadStats] = useState<BulkUploadResponse | undefined>(undefined);
  ...
  const handleBulkUpload = async (data: Array<Record<string, unknown>>) => {
    if (!formData.plant_id) {
    ...
    setBulkUploadSuccess(undefined);
    setBulkUploadStats(undefined);

    try {
      // Transform data to match API expectations
      const transformedData = data.map(row => {
        // Add plant_id to each record and ensure required fields are present
        return {
          plant_id: formData.plant_id,
          date: row.date as string || format(new Date(), 'yyyy-MM-dd'),
          code: row.code as string || '',
          product: row.product as string || '',
          truck_no: row.truck_no as string || '',
          bill_no: row.bill_no as string || '',
          party_name: row.party_name as string || '',
          rate: typeof row.rate === 'string' ? parseFloat(row.rate) : (row.rate as number || 0),
          mv: typeof row.mv === 'string' ? parseFloat(row.mv) : (row.mv as number || 0),
          oil: typeof row.oil === 'string' ? parseFloat(row.oil) : (row.oil as number || 0),
          fiber: typeof row.fiber === 'string' ? parseFloat(row.fiber) : (row.fiber as number || 0),
          starch: typeof row.starch === 'string' ? parseFloat(row.starch) : (row.starch as number || 0),
          maize_rate: typeof row.maize_rate === 'string' ? parseFloat(row.maize_rate) : (row.maize_rate as number || 0),
        };
      });

      const response = await plantDataApi.bulkUploadPlantRecords(
        formData.plant_id,
        transformedData as Array<Omit<PlantRecord, 'id' | 'plant' | 'created_at' | 'updated_at' | 'dm' | 'rate_on_dm' | 'oil_value' | 'net_wo_oil_fiber' | 'starch_per_point' | 'starch_value' | 'grain' | 'doc'>>
      );

      setBulkUploadStats(response);
      setBulkUploadSuccess(response.failed === 0);

      if (response.failed === 0) {
        toast.success(`Successfully uploaded ${response.success} records`);
      } else {
        toast.warning(`Uploaded ${response.success} records with ${response.failed} failures`);
      }
    } catch (error) {
      console.error('Error uploading data:', error);
      setBulkUploadSuccess(false);
      if (error instanceof AxiosError) {
        toast.error(error.response?.data?.detail || 'Failed to upload data');
      } else {
    ...
    <div className="container mx-auto p-4">
      ...
      {!isEditing && (
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
          ...
          <div className="mt-6">
            {/* Plant Selection - Always visible regardless of tab */}
            <Card className="mb-6">
              <CardHeader>
      ...
      {activeTab === "excel" && !isEditing ? (
        <ExcelUploader
          templateFields={requiredFields}
          onDataParsed={handleBulkUpload}
          isProcessing={isProcessingBulkUpload}
          uploadSuccess={bulkUploadSuccess}
          uploadStats={bulkUploadStats}
        />
      ) : (
